...
def train_model(model: nn.Module, num_epochs, train_dataloader, valid_dataloader, optimizer, loss_fn, results_path):
    ...
    ids_train, preds_train, labels_train = _get_predictions(model=model, dataloader=train_dataloader)
    _save_preds(ids=ids_train, preds=preds_train, trues=labels_train, 
                 save_to=os.path.join(results_training_path, 'preds_train_data.csv'))
    
    #   Save preds for valid data
    ids_valid, preds_valid, labels_valid = _get_predictions(model=model, dataloader=valid_dataloader)
    _save_preds(ids=ids_valid, preds=preds_valid, trues=labels_valid, 
